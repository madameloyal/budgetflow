...
async def import_qonto(file: UploadFile = File(...), session: str = None):
    ...
    try:
        ...
        for base_t in s_dept_tabs:
            ...
            for i, row in enumerate(vals):
                ...
                if not ligne or ligne.startswith("(ligne") or is_total_row(section, ligne):
                    continue
                reset_data.append({"range": f"{full_t}!I{i+1}:J{i+1}", "values": [[0, 0]]})
    ...
    if accumulate:
        try:
            write_data = [
                {"range": f"{s_prefix}{tab}!I{row}:J{row}", "values": [[ht, ttc]]}
    ...
    try:
        from datetime import datetime
        ensure_match_log_tab(svc, ml_tab)
        svc.values().clear(spreadsheetId=SHEET_ID, range=f"{ml_tab}!A:J").execute()
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
