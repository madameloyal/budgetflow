...
def ensure_match_log_tab(svc):
    """Create MATCH_LOG tab if it doesn't exist, with headers."""
    try:
        meta = svc.get(spreadsheetId=SHEET_ID).execute()
        existing = {s["properties"]["title"] for s in meta["sheets"]}
        if MATCH_LOG_TAB not in existing:
            svc.batchUpdate(
                spreadsheetId=SHEET_ID,
                body={"requests": [{"addSheet": {"properties": {"title": MATCH_LOG_TAB}}}]}
            ).execute()
            svc.values().update(
                spreadsheetId=SHEET_ID, range=f"{MATCH_LOG_TAB}!A1",
...
def append_match_log(svc, dept: str, section: str, ligne: str, fourn: str, note: str, date: str, ht: float, ttc: float, source: str):
    """Append a row to the MATCH_LOG tab."""
    from datetime import datetime
    try:
        ensure_match_log_tab(svc)
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        svc.values().append(
            spreadsheetId=SHEET_ID,
            range=f"{MATCH_LOG_TAB}!A:J",
            valueInputOption="RAW",
...
def get_match_log(dept: str = "", ligne: str = ""):
...
def assign_transaction(payload: AssignPayload):
    ...
    try:
        ...
        target_row_num = None
        current_reel_ht  = 0.0
        current_reel_ttc = 0.0
        for i, row in enumerate(dept_rows):
            row = list(row) + [""] * 12
            if str(row[COL["ligne"]]).strip() == payload.ligne.strip():
                target_row_num = i + 1
                current_reel_ht  = safe_float(row[COL["reel_ht"]])
                current_reel_ttc = safe_float(row[COL["reel_ttc"]])
                break

        if target_row_num is None:
            raise HTTPException(status_code=404, detail=f"Ligne '{payload.ligne}' not found in {tab}")

        new_reel_ht  = current_reel_ht  + abs(ht_val)
        new_reel_ttc = current_reel_ttc + abs(ttc_val)
        # Write both reel_ht (col I) and reel_ttc (col J)
        svc.values().update(
            spreadsheetId=SHEET_ID,
            range=f"{tab}!I{target_row_num}:J{target_row_num}",
            valueInputOption="USER_ENTERED",
            body={"values": [[new_reel_ht, new_reel_ttc]]}
        ).execute()
        ...
        append_match_log(svc, tab, tx_section, payload.ligne, tx_fourn, tx_note, tx_date, abs(ht_val), abs(ttc_val), "manual")

        return {"status": "ok", "assigned": payload.ligne, "dept": tab,
                "ht_added": abs(ht_val), "ttc_added": abs(ttc_val),
                "new_reel_ht": new_reel_ht, "new_reel_ttc": new_reel_ttc}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def dissociate_transaction(payload: DissociatePayload):
    ...
    try:
        ...
        # 1. Subtract HT/TTC from budget line réel
        dept_resp = svc.values().get(
            spreadsheetId=SHEET_ID,
            range=f"{tab}!A:L",
            valueRenderOption="UNFORMATTED_VALUE"
        ...
        new_reel_ht = max(0, current_reel_ht - abs(payload.ht))
        new_reel_ttc = max(0, current_reel_ttc - abs(payload.ttc))
        svc.values().update(
            spreadsheetId=SHEET_ID,
            range=f"{tab}!I{target_row_num}:J{target_row_num}",
            valueInputOption="USER_ENTERED",
        ...
        # 2. Remove matching row from MATCH_LOG
        ensure_match_log_tab(svc)
        ...
        for i, row in enumerate(log_rows):
            if i == 0:
                continue
            row = list(row) + [""] * 10
            if (str(row[0]).strip().upper() == tab and
        ...
        if log_row_to_delete is not None:
            ...
            log_sheet_id = next(
                (s["properties"]["sheetId"] for s in sheet_meta["sheets"]
                 if s["properties"]["title"] == MATCH_LOG_TAB), None
            )
        ...
        # 3. Re-add to QONTO_UNMATCHED
        ensure_match_log_tab(svc)  # reuse to ensure tabs exist
        unmatched_row = [
            tab, "", payload.fourn, payload.note, "",
...
async def import_qonto(file: UploadFile = File(...)):
    ...
    try:
        ...
        tabs_to_create = [t for t in [RAW_TAB, UNMATCHED_TAB, MATCH_LOG_TAB] if t not in existing_tabs]
        if tabs_to_create:
            svc.batchUpdate(
                spreadsheetId=SHEET_ID,
    ...
    match_log_rows = []  # rows to append to MATCH_LOG
    ...
    for raw in raw_rows:
        ...
        if match:
            ...
            match_log_rows.append([target_tab, match.get("section",""), match["ligne"], fourn, note, date, abs(ht), abs(ttc), "auto"])
            results["matched"] += 1
        else:
    ...
    # Write MATCH_LOG (clear + rewrite for this import)
    try:
        from datetime import datetime
        ensure_match_log_tab(svc)
        svc.values().clear(spreadsheetId=SHEET_ID, range=f"{MATCH_LOG_TAB}!A:J").execute()
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        log_write = [MATCH_LOG_HEADERS] + [
            row + [timestamp] for row in match_log_rows
        ]
        svc.values().update(
            spreadsheetId=SHEET_ID, range=f"{MATCH_LOG_TAB}!A1",
            valueInputOption="RAW", body={"values": log_write}
        ).execute()
    except Exception as e:
        print(f"Warning: could not write MATCH_LOG: {e}")
